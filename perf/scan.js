...
suite
  .add('most', function (deferred) {
    runners.runMost(deferred, most.reduce(passthrough, 0, most.scan(sum, 0, most.from(a))))
  }, options)
...
function arrayScan (f, initial, a) {
  ...
  return a.map(function (x) {
    return result = f(result, x) // eslint-disable-line no-return-assign
  })
}
